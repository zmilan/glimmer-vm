...
// import { buildStatement, buildExpression } from './print';

// type Intermediate = Node.ExpressionNode | Node.Ident | Node.Args | Node.Positional | Node.Named;

// function verifyLoc(source: string, expected: string, loc: Node.Location) {
...
// function verifyExpression(ast: Intermediate): boolean {
//   let printed: string = buildExpression(ast);

//   switch(ast.type) {
//     case Node.Expression.Path:
//       return verifyLoc(printed, (ast as Node.Path).loc);
...
//       return compactJoin([ buildExpression(sexpr.path), buildExpression(sexpr.args.positional), buildExpression(sexpr.args.named) ], ' ')
...
// export function verifyStatement(ast: TopLevel): string {
//   if(!ast) {
//     return '';
//   }
...
//       }
//       output.push('>');
//       output.push.apply(output, buildTopLevels(element.children));
//       output.push('</', element.tag, '>');
//       break;
